...
const HMAC_MASTER = Deno.env.get('WORLDMIND_HMAC_SECRET') ?? '';
const MAX_SKEW_SECONDS = Number(Deno.env.get('WORLDMIND_MAX_SKEW_SECONDS') ?? '90');

if (!SUPABASE_URL || !SERVICE_ROLE_KEY || !HMAC_MASTER) {
  console.error('world-agent missing required server secrets');
}
...
Deno.serve(async (req: Request): Promise<Response> => {
  if (req.method === 'GET') {
    return json({ ok: true, service: 'specter-world-agent', version: 1 });
  }
  if (req.method !== 'POST') return json({ error: 'method_not_allowed' }, 405);
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY || !HMAC_MASTER) return json({ error: 'server_not_configured' }, 503);
  ...
  try {
    ...
    const canonicalPayload = canonicalJson(proposal.payload);
    const payloadSha256 = await sha256Hex(new TextEncoder().encode(canonicalPayload));
    if (!timingSafeEqualHex(payloadSha256, proposal.payloadSha256.toLowerCase())) {
      return json({ error: 'payload_hash_mismatch' }, 400);
    }

    const bodySha256 = await sha256Hex(raw);
    const expectedSignature = await signAgent(proposal.agentId, `${proposal.agentId}\n${ts}\n${nonce}\n${bodySha256}`);
    if (!timingSafeEqualHex(expectedSignature, signature)) return json({ error: 'bad_signature' }, 401);

    const { data: agent, error: agentError } = await db
      .from('world_agents')
      .select('id,enabled,capabilities,budget')
      .eq('id', proposal.agentId)
      .maybeSingle();
    if (agentError) throw agentError;
    if (!agent?.enabled) return json({ error: 'agent_disabled_or_unknown' }, 403);
    if (!Array.isArray(agent.capabilities) || !agent.capabilities.includes(proposal.capability)) {
      return json({ error: 'capability_denied' }, 403);
    }

    const provenanceError = validateProvenance(proposal.provenance);
    if (provenanceError) return json({ error: provenanceError }, 400);

    // Replay barrier is inserted only after signature + capability checks.
    ...
    if (proposal.parentRevisionId !== undefined && proposal.parentRevisionId !== current) {
      return json({ error: 'stale_parent_revision', currentRevisionId: current }, 409);
    }
    ...
    const { data: revision, error: revisionError } = await db
      ...
      .select('id,revision_no,status,parent_id')
      .single();
    ...
    await db.from('world_events').insert({
      ...
      agent_id: proposal.agentId,
      revision_id: revision.id,
      payload: { capability: proposal.capability, kind: proposal.kind, objectKey: proposal.objectKey },
    });
    ...
    return json({
      ...
      parentRevisionId: revision.parent_id,
      status: revision.status,
      payloadSha256: proposal.payloadSha256.toLowerCase(),
    }, 202);
  } catch (error) {
...
function safeId(value: unknown, max: number): value is string {
...
async function signAgent(agentId: string, message: string): Promise<string> {
  const encoder = new TextEncoder();
  const master = await crypto.subtle.importKey('raw', encoder.encode(HMAC_MASTER), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
  const derivedRaw = await crypto.subtle.sign('HMAC', master, encoder.encode(agentId));
  const derived = await crypto.subtle.importKey('raw', derivedRaw, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
  const sig = await crypto.subtle.sign('HMAC', derived, encoder.encode(message));
